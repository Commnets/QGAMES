...
/**	
 *	@file	
 *	File: dwscoreobjects.hpp \n
 *	Framework: Commty Game Library (CGL) \n
 *	Author: Ignacio Cea Fornies (Community Networks) \n
 *	Creation Date: 04/01/2019 \n
 *	Description: Defines the base for the score objects accesible for a dragonwind game.
 *	Versions: 1.0 Initial
 */

#ifndef __DRAGONWIND_SCOREOBJECTS__
#define __DRAGONWIND_SCOREOBJECTS__
...
{
	/** Class to represent the energy of the player. 
		...
		That form has to ha two frames: One representing the bar empty (first) and the other other full. 
	...
	class EntityEnergy : public QGAMES::ScoreObject
	{
		...
		virtual Entity* clone () const
							{ return (new EntityEnergy (id () - __DRAGONWIND_ENTITYENERGYSCOBJID__, _energyForm)); }
	...
	class NinjaLives : public QGAMES::ScoreObject
	{
		public:
		// The score object can be set for male or female aspect...male by default!
		NinjaLives (int tA = 0, int lF = __DRAGONWIND_NINJALIVESFORM__);

		/** @see parent. */
		virtual Entity* clone () const
							{ return (new NinjaLives (_typeArtist, _livesForm)); }
	...
	/** Class to represent one thing being carried by the ninja. 
	...
	class NinjaThingBeingCarried : public QGAMES::ScoreObject
	{
		public:
		NinjaThingBeingCarried (int pId = 0, int tC = __DRAGONWIND_NINJATHINGTOCARRYFORM__);

		/** @see parent. */
		virtual Entity* clone () const
						{ return (new NinjaThingBeingCarried (id () - __DRAGONWIND_NINJATHINGBEINGCARRIEDID__)); }
	...
	class NinjaThingsBeingCarried : public QGAMES::ScoreObject
	{
		public:
		NinjaThingsBeingCarried (int nT = __DRAGONWIND_MAXNUMBERTHINGSBEINGCARRIED__);
		~NinjaThingsBeingCarried ();
		...
		virtual NinjaThingBeingCarried* createThingBeingCarried (int id)
								{ return (new NinjaThingBeingCarried (id)); }

		protected:
		// Implementation
		std::vector <NinjaThingBeingCarried*> _things;
	...
	class ShootsLeftCounter : public QGAMES::ScoreObjectNumber
	{
		...
		/** @see parent. */
		virtual Entity* clone () const
								{ return (new ShootsLeftCounter (id () - __DRAGONWIND_ENTITYSHOOTINGSSTILLLEFTID__)); }
