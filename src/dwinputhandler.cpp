...
// ---
DRAGONWIND::InputHandlerBehaviour::InputHandlerBehaviour ()
...
// ---
void DRAGONWIND::InputHandlerBehaviour::onKeyboardStatus (const std::vector <bool>& kS)
{
	...
	{
		case 0x01:
			evntId = __DRAGONWIND_MOVINGUPEVENT__;
			break;
		case 0x02:
			evntId = __DRAGONWIND_MOVINGRIGHTEVENT__;
			break;
		case 0x03:
			evntId = __DRAGONWIND_MOVINGUPRIGHTEVENT__;
			break;
		case 0x04:
			evntId = __DRAGONWIND_MOVINGDOWNEVENT__;
			break;
		case 0x06:
			evntId = __DRAGONWIND_MOVINGDOWNRIGHTEVENT__;
			break;
		case 0x08:
			evntId = __DRAGONWIND_MOVINGLEFTEVENT__;
			break;
		case 0x09:
			evntId = __DRAGONWIND_MOVINGUPLEFTEVENT__;
			break;
		case 0x0C:
			evntId = __DRAGONWIND_MOVINGDOWNLEFTEVENT__;
			break;
		default:
			// Take into account that an strange combinations of keys, will stop the movement...
			evntId = __DRAGONWIND_NOMOVINGEVENT__;
			break;
	}
	...
	else
	{
		if (_force != 0)
		{
			...
			inputHandler () -> notify (QGAMES::Event (__DRAGONWIND_FIREEVENT__, _notificationData));
	...
	if (kS [_keys [__DRAGONWIND_KEYJUMP__]])
		_impulse++;
	else
	{
		if (_impulse != 0)
		{
			...
			_notificationData -> _leftOn = kS [_keys [__QGAMES_KEYLEFT__]];
			_notificationData -> _rightOn = kS [_keys [__QGAMES_KEYRIGHT__]];
			inputHandler () -> notify (QGAMES::Event (__DRAGONWIND_JUMPEVENT__, _notificationData));
...
// ---
void DRAGONWIND::InputHandlerBehaviour::manageKeyOnState (QGAMES::GameState* st, int kP)
{
	// The state is not used
	// The observer will decide waht to do with the event...

	if (kP == _keys [__DRAGONWIND_KEYCATCHLEAVE__])
		inputHandler () -> notify (QGAMES::Event (__DRAGONWIND_CATCHLEAVESOMETHINGEVENT__, NULL));
	if (kP == _keys [__DRAGONWIND_KEYITERATE__])
		inputHandler () -> notify (QGAMES::Event (__DRAGONWIND_ITERATEELEMENTSEVENT__, NULL));
}

// ---
void DRAGONWIND::InputHandlerBehaviour::manageJoystickMovementOnState 
...
// ---
void DRAGONWIND::InputHandlerBehaviour::manageJoystickButtonOnState 
