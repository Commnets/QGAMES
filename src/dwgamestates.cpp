#include <Dragonwindlike/dwgamestates.hpp>
#include <DragonwindLike/dwgame.hpp>
#include <DragonwindLike/dwworlds.hpp>
#include <DragonwindLike/dwscenes.hpp>
#include <DragonwindLike/dwinputhandler.hpp>

// ---
void DRAGONWIND::InitializeDragonWindGameState::onEnter ()
{
	QGAMES::GameState::onEnter ();

	DRAGONWIND::Game* aG = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (aG); // Just in case...

	aG -> initializeEntitiesAndWorlds ();
}

// ---
void DRAGONWIND::InitializeDragonWindGameState::updatePositions ()
{
	notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this));
}

// ---
void DRAGONWIND::ShowingCurrentPlayerNameGameState::onEnter ()
{
	QGAMES::ShowingStandardTextGameState::onEnter ();

	// If there is no player name already introduced, then 
	// a GUI system is started to ask for it...
	DRAGONWIND::Game* g = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (g); // Just in case...
	if (g -> playerName () == std::string (__NULL_STRING__))
	{
		_guiSystem = game () -> guiSystemBuilder () -> system (__DRAGONWIND_PLAYERNAMEGUISYSTEM__);
		QGAMES::CompositeWidget* cW = dynamic_cast <QGAMES::CompositeWidget*> (_guiSystem -> mainWidget ());
		assert (cW); // Just in case...
		QGAMES::IntroWidget* iW = 
			dynamic_cast <QGAMES::IntroWidget*> (cW -> widget (__DRAGONWIND_PLAYERNAMEOPTIONID__));
		assert (iW); // Just in case...
		QGAMES::IteratingLabelWidget* aW = 
			dynamic_cast <QGAMES::IteratingLabelWidget*> (cW -> widget (__DRAGONWIND_GUISYSTEMTYPEARTISTWDT__));
		assert (aW); // Just in case...

		// The position of the main widget is defined with the same
		// attributes used when the name has already been introduced!
		int tW = iW -> width (); int tH = iW -> height ();
		QGAMES::Vector adj = QGAMES::Vector::_cero;
		if (_properties._reference == QGAMES::ShowingTextGameState::Properties::PosReference::_CENTER)
			adj = QGAMES::Vector (__BD (tW >> 1), __BD (tH >> 1), __BD 0);
		else if (_properties._reference == QGAMES::ShowingTextGameState::Properties::PosReference::_BOTTONRIGHT)
			adj = QGAMES::Vector (__BD tW, __BD tH, __BD 0);
		_guiSystem -> mainWidget () -> setPosition (QGAMES::Position (
				__BD game () -> mainScreen () -> visualWidth () * _properties._position.posX (),
				__BD game () -> mainScreen () -> visualHeight () * _properties._position.posY (),
				__BD 0) - adj); // The position declared under the properties structure is always a percentage...

		// Sets the default type of artist to play...
		aW -> setCurrentValue (g -> artistType ());

		observe (_guiSystem -> mainWidget ());

		// Just to know when the intro is pressed to finish the state
		// and then continue with the game...
		observe (game () -> inputHandler ());
	}
}

// ---
void DRAGONWIND::ShowingCurrentPlayerNameGameState::onExit ()
{
	QGAMES::ShowingStandardTextGameState::onExit ();

	if (_guiSystem)
	{
		unObserve (game () -> inputHandler ());

		DRAGONWIND::Game* g = dynamic_cast <DRAGONWIND::Game*> (game ());
		assert (g); // Just in case...
		QGAMES::CompositeWidget* cW = dynamic_cast <QGAMES::CompositeWidget*> (_guiSystem -> mainWidget ());
		assert (cW);

		// Get the name introduced...
		std::string txt = ((QGAMES::IntroWidget*) cW -> widget (__DRAGONWIND_PLAYERNAMEOPTIONID__)) -> text ();
		QGAMES::trim (txt); // Erase the spaces...if any
		g -> setPlayerName ((txt == std::string (__NULL_STRING__)) ? std::string ("AAA") : txt); 
		// Sets the name introduced or a default one if none...

		// Get the type of artist selected...
		g -> setArtistType (((QGAMES::IteratingLabelWidget*) 
			cW -> widget (__DRAGONWIND_GUISYSTEMTYPEARTISTWDT__)) -> currentValue ());

		// ...and finally the GUI system is deleted...
		deleteGUISystem ();
	}
}

// ---
void DRAGONWIND::ShowingCurrentPlayerNameGameState::processEvent (const QGAMES::Event& evnt)
{
	QGAMES::ShowingStandardTextGameState::processEvent (evnt);

	// When the GUI system is active, and the INTRO key is pressed, the state finishes only
	// when the name of the player is not null...
	if (_guiSystem)
	{
		if (evnt.code () == __QGAMES_KEYBOARDEVENT__ &&
			((QGAMES::KeyBoardEventData*) evnt.data ()) -> _internalCode == QGAMES::KeyCode::QGAMES_RETURN &&
			!((QGAMES::KeyBoardEventData*) evnt.data ()) -> _on && // The key has been released!!
			((QGAMES::IntroWidget*) ((QGAMES::CompositeWidget*) _guiSystem -> mainWidget ()) -> 
				widget (__DRAGONWIND_PLAYERNAMEOPTIONID__)) -> text () != std::string (__NULL_STRING__))
			notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this));
	}
}

// ---
void DRAGONWIND::ShowingCurrentPlayerNameGameState::actualizeText ()
{
	QGAMES::ShowingStandardTextGameState::actualizeText ();

	if (!_guiSystem)
	{
		assert (dynamic_cast <DRAGONWIND::Game*> (game ()));
		// It has to be an DRAGONWIND game to access to the number of the player...

		std::string str = _textString + std::string (" ") + // An space after...
			((DRAGONWIND::Game*) game ()) -> playerName ();
		_text -> setMaxTextLength (str.length ()); _text -> setText (str);
	}
	else
	{
		_text -> setMaxTextLength (1); // Can't be 0...
		_text -> setText (std::string (__NULL_STRING__));
	}
}

// ---
void DRAGONWIND::ShowingCurrentPlayerNameGameState::deleteGUISystem ()
{
	if (_guiSystem)
		unObserve (_guiSystem -> mainWidget ());

	delete (_guiSystem);
	_guiSystem = NULL;
}

// ---
void DRAGONWIND::ShowingCurrentWorldNameGameState::actualizeText ()
{
	assert (dynamic_cast <DRAGONWIND::World*> ((__AGM game ()) -> activeWorld ()));
	// Just in case...It has to be a DRAGONWIND::World

	QGAMES::ShowingTextGameState::actualizeText ();
	std::string str = _textString + std::string (" ") + // A space after...
		((DRAGONWIND::World*) (__AGM game ()) -> activeWorld ()) -> name ();
	_text -> setMaxTextLength (str.length ()); _text -> setText (str);
}

// ---
void DRAGONWIND::ShowingCurrentSceneNameGameState::actualizeText ()
{
	assert (dynamic_cast <DRAGONWIND::Scene*> ((__AGM game ()) -> activeWorld () -> activeScene ()));
	// Jus in case...It has to be a DRAGONWIND::Scene

	QGAMES::ShowingTextGameState::actualizeText ();
	std::string str = _textString + std::string (" ") + // A space after...
		((DRAGONWIND::Scene*) (__AGM game ()) -> activeWorld () -> activeScene ()) -> name ();
	_text -> setMaxTextLength (str.length ()); _text -> setText (str);
}

// ---
DRAGONWIND::CreditTitlesGameState::CreditTitlesGameState 
		(int id, int nT, const std::string& cT, 
		 const QGAMES::CreditTitlesGameState::Properties& prps, QGAMES::GameState* nS)
	: QGAMES::CreditTitlesGameState (id, prps, nS),
		_numberTitle (nT),
		_contentTitle (cT),
		_logoGameState (NULL)
{
	// To be able to configure the logo behind...
	_logoGameState = dynamic_cast <QGAMES::ShowingFixFormGameState*> (nestedState ());
	// It could be null, meaning that the state hasn't been well defined at all...
}

// ---
void DRAGONWIND::CreditTitlesGameState::drawOn (QGAMES::Screen* s)
{
	if (_logoGameState != NULL)
	{
		QGAMES::ShowingFixFormGameState::Properties prps = _logoGameState -> properties ();
		prps._fade = (_titleToPaint == _numberTitle) ? _titleStatus [_titleToPaint] -> _fadeGrade : 0;
		_logoGameState -> setProperties (prps); // Change the fade according with the title shown...
	}

	QGAMES::CreditTitlesGameState::drawOn (s);
}

// ---
QGAMES::ScoreObjectText* DRAGONWIND::CreditTitlesGameState::createText (const std::string& txt)
{
	QGAMES::ScoreObjectText* result = NULL;

	bool last = (txt == properties ()._titles [properties ()._titles.size () - 1]);
	if (txt.find (_contentTitle) != txt.npos) // It the text to show is the one defined as the main title... 
		result = game () -> textBuilder () -> createText
			(game () -> textBuilder () -> nextSizeTo ( // Even bigger...
				(game () -> textBuilder () -> nextSizeTo // Bigger and green...
					(game () -> textBuilder () -> inColor (properties ()._font, __QGAMES_GREENCOLOR__)))), txt);
	else 
	if (last) // The last text is show in a green always...
		result = game () -> textBuilder () -> createText 
			(game () -> textBuilder () -> inColor (properties ()._font, __QGAMES_GREENCOLOR__), txt);
	else 
		result = QGAMES::CreditTitlesGameState::createText (txt);

	return (result);
}

// ---
std::multimap <std::string, QGAMES::SetOfOpenValues> DRAGONWIND::ShowingHallOfFameGameState::orderHallOfFame 
	(const std::multimap <std::string, QGAMES::SetOfOpenValues>& hF)
{
	// The result is almost what is being received...
	// But the index of the map has to be the score instead the number of the player...
	std::multimap <int, QGAMES::SetOfOpenValues, std::greater <int>> pR;
	for (std::multimap <std::string, QGAMES::SetOfOpenValues>::const_iterator i = hF.begin ();
			i != hF.end (); i++)
		pR.insert (std::multimap <int, QGAMES::SetOfOpenValues, std::greater <int>>::value_type 
			((*i).second.openValue (0).intValue (), (*i).second)); // The score in the openValu number 0..
	// Because the mulmap is ordered using the score...

	// The result is reasy to translate into...
	std::multimap <std::string, QGAMES::SetOfOpenValues> result;
	for (std::multimap <int, QGAMES::SetOfOpenValues, std::greater <int>>::const_iterator i = pR.begin (); i != pR.end (); i++)
		result.insert (std::multimap <std::string, QGAMES::SetOfOpenValues>::value_type (std::to_string ((*i).first), (*i).second));

	return (result);
}

// ---
QGAMES::ScoreObjectText* DRAGONWIND::ShowingHallOfFameGameState::createTextFor 
	(const std::string& eK, const QGAMES::SetOfOpenValues& sV)
{
	std::string scr = std::to_string (sV.openValue (0).intValue ());
	if (scr.length () < 5) 
		scr = std::string ("00000").substr (0, 5 - scr.length ()) + scr;
	return (game () -> textBuilder () -> createText 
		(properties ()._font, (scr + std::string ("  ") + sV.openValue (1).strValue ())));
}

// ---
void DRAGONWIND::SelectionGameState::initializeGUISystem ()
{
	QGAMES::StandardInitialSelectionOptionsGameState::initializeGUISystem ();

	QGAMES::CompositeWidget* mWdt = 
		dynamic_cast <QGAMES::CompositeWidget*> (currentGUISystem () -> mainWidget ());
	// It is not needed to assert it as it has been already done at parent level
	if (mWdt -> id () == __QGAMES_STDGUISYSTEMMAINWIDGET__)
	{
		DRAGONWIND::Game::Conf* cfg = dynamic_cast <DRAGONWIND::Game::Conf*> (_configuration);
		assert (cfg); // Just in case...
		((QGAMES::IteratingLabelWidget*) // The same type of artist for all of them...
			(mWdt -> widget (__DRAGONWIND_GUISYSTEMTYPEARTISTWDT__))) -> setCurrentValue (cfg -> artistType (1));
	}
	else
	if (mWdt -> id () == __QGAMES_STDGUISYSTEMMAINCONFWIDGET__)
	{
		std::vector <int> controlKeys = _configuration -> controlKeys ();
		// It is not needed to assert the size at it has already been done at parent level...
		((QGAMES::IteratingLabelWidget*) 
			(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYITERATEWDT__))) -> 
				setCurrentValue (_keyCodesConverterInv [(QGAMES::KeyCode) controlKeys [__DRAGONWIND_KEYITERATE__]]);
		((QGAMES::IteratingLabelWidget*) 
			(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYCATCHLEAVEWDT__))) -> 
				setCurrentValue (_keyCodesConverterInv [(QGAMES::KeyCode) controlKeys [__DRAGONWIND_KEYCATCHLEAVE__]]);
		((QGAMES::IteratingLabelWidget*) 
			(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYJUMPWDT__))) -> 
				setCurrentValue (_keyCodesConverterInv [(QGAMES::KeyCode) controlKeys [__DRAGONWIND_KEYJUMP__]]);
	}
}

// ---
void DRAGONWIND::SelectionGameState::finalizeGUISystem ()
{
	QGAMES::StandardInitialSelectionOptionsGameState::finalizeGUISystem ();

	QGAMES::CompositeWidget* mWdt = 
		dynamic_cast <QGAMES::CompositeWidget*> (currentGUISystem () -> mainWidget ());
	// It is not needed to assert it as it has been already done at parent level
	if (mWdt -> id () == __QGAMES_STDGUISYSTEMMAINWIDGET__)
	{
		DRAGONWIND::Game::Conf* cfg = dynamic_cast <DRAGONWIND::Game::Conf*> (_configuration);
		assert (cfg); // Just in case...
		for (int i = 1; i <= cfg -> numberPlayers (); i++)
			cfg -> setArtistType (i,
				((QGAMES::IteratingLabelWidget*) // All of them are set as male or female...
					(mWdt -> widget (__DRAGONWIND_GUISYSTEMTYPEARTISTWDT__))) -> currentValue ());
	}
	else
	if (mWdt -> id () == __QGAMES_STDGUISYSTEMMAINCONFWIDGET__)
	{
		std::vector <int> controlKeys (9);
		controlKeys = _configuration -> controlKeys ();
		controlKeys [__DRAGONWIND_KEYITERATE__] = 
			_keycodesConverter [((QGAMES::IteratingLabelWidget*) 
				(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYITERATEWDT__))) -> currentValue ()];
		controlKeys [__DRAGONWIND_KEYCATCHLEAVE__] = 
			_keycodesConverter [((QGAMES::IteratingLabelWidget*) 
				(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYCATCHLEAVEWDT__))) -> currentValue ()];
		controlKeys [__DRAGONWIND_KEYJUMP__] = 
			_keycodesConverter [((QGAMES::IteratingLabelWidget*) 
				(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYJUMPWDT__))) -> currentValue ()];
		_configuration -> setControlKeys (controlKeys);
	}
}

// ---
int DRAGONWIND::SelectionGameState::SelectionHandler::whenWidgetSelected (QGAMES::Widget* wdt)
{
	int result = QGAMES::StandardInitialSelectionOptionsGameState::SelectionHandler::whenWidgetSelected (wdt);
	if (result != -1) // Something has happened already?
		return (result);

	switch (wdt -> id ())
	{
		case __DRAGONWIND_GUISYSTEMINSTRUCTIONSWDT__:
			result = wdt -> id (); // Time to exit the menu, looking for the instructions...
			break;

		// In these cases lets analize whether there is any key duplicated
		// If it is the case, then an information message should be shown!
		case __DRAGONWIND_GUISYSTEMCFGKEYITERATEWDT__:
		case __DRAGONWIND_GUISYSTEMCFGKEYCATCHLEAVEWDT__:
		case __DRAGONWIND_GUISYSTEMCFGKEYJUMPWDT__:
			{
				if (owner () -> informationText () != std::string (__NULL_STRING__))
				{
					QGAMES::CompositeWidget* mWdt = 
						dynamic_cast <QGAMES::CompositeWidget*> (owner () -> currentGUISystem () -> mainWidget ());
					assert (mWdt); // It has to be...
					std::vector <int> nowKeys (3);
					nowKeys [__DRAGONWIND_KEYITERATE__ - 7] = ((QGAMES::IteratingLabelWidget*) 
							(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYITERATEWDT__))) -> currentValue ();
					nowKeys [__DRAGONWIND_KEYCATCHLEAVE__ - 7] = ((QGAMES::IteratingLabelWidget*) 
							(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYCATCHLEAVEWDT__))) -> currentValue ();
					nowKeys [__DRAGONWIND_KEYJUMP__ - 7] = ((QGAMES::IteratingLabelWidget*) 
							(mWdt -> widget (__DRAGONWIND_GUISYSTEMCFGKEYJUMPWDT__))) -> currentValue ();

					int nE = nowKeys.size ();
					auto end = nowKeys.end ();
					for (auto it = nowKeys.begin (); it != end; ++it)
						end = std::remove (it + 1, end, *it);
					nowKeys.erase(end, nowKeys.end ());
					owner () -> showInformationText 
						(std::string ((nowKeys.size () != nE) ? "Take care! Keys repeated" : " "));
				}
			}

			break;

		default:
			break; // When the rest of the widgets are selected what the standard behaviour does is more than enough
	}

	return (result);
}

// ---
DRAGONWIND::PlayingGameState* DRAGONWIND::PlayingControlGameState::playingState ()
{
	DRAGONWIND::PlayingGameState* result = NULL;
	for (QGAMES::GameStates::const_iterator i = states ().begin (); i != states ().end () && !result; i++)
		if (dynamic_cast <DRAGONWIND::PlayingGameState*> ((*i).second))
			result = (DRAGONWIND::PlayingGameState*) ((*i).second);
	return (result);
}

// ---
void DRAGONWIND::PlayingControlGameState::onEnter ()
{
	QGAMES::GameStateControl::onEnter ();

	DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (dG); // Just in case...
	DRAGONWIND::Game::Conf* dGConf = dynamic_cast <DRAGONWIND::Game::Conf*> (dG -> configuration ());
	assert (dGConf); // Just in case...

	dG -> addScoreObjects ();

	_mainArtist = dG -> mainArtist (); 
	assert (_mainArtist); // Just in case...

	int nCS = dG -> currentScene ();
	dG -> setWorld (dG -> currentWorld ());
	dG -> activeWorld () -> setScene (nCS);
	DRAGONWIND::Scene* dScn = dynamic_cast <DRAGONWIND::Scene*> (dG -> activeWorld () -> activeScene ());
	assert (dScn); // Just in case, only DRAGONWIND::Scenes are allowed...
	dG -> activeWorld () -> initialize ();

	// The position of the main artist will depend on whether is finishes last time...
	QGAMES::Position lPP = dG -> lastPlayerPosition ();
	if (lPP == QGAMES::Position::_cero) // No last position was set...
	{
		DRAGONWIND::ToKnowTileFrameInformation* tInf =
			dynamic_cast <DRAGONWIND::ToKnowTileFrameInformation*> (dG -> tileFrameInformation ());
		assert (tInf); // Just in case...
		QGAMES::PlatformMap::ObjectLocations pP = // If there is no inicial point defined for that scene, 0 will be returned
			dScn -> positionsForObject (tInf -> tileFrameAssociatedToInitPoint (dG -> currentInitPoint (nCS)));
		if (!pP.empty ()) _mainArtist -> setPosition ((*pP.begin ())._position); // Initial point defined?
		else _mainArtist -> setPosition (QGAMES::Position (__BD 24, __BD 24, __BD 0)); // A generical one (1,1 tile)
	}
	else // Last position was set...
		_mainArtist -> setPosition (lPP);
	_mainArtist -> setFixBasePosition (); // Just in case...to be calculated along the game...

	// Fix the visualization over the main character
	QGAMES::Screen* scr = dG -> mainScreen ();
	QGAMES::ToFixScreenOnArtistInSceneWithBackground* tFS =
		new QGAMES::ToFixScreenOnArtistInSceneWithBackground (scr, _mainArtist, __BD 0.4, __BD 0.4, __BD 0);
	scr -> setFix (tFS);

	// It will depend on the attributes of the current screen...
	QGAMES::Vector dScnM = dScn -> movement ();
	tFS -> setConsiderXAxis (dScnM.posX () > 0 ? true : false); 
	tFS -> setConsiderYAxis (dScnM.posY () > 0 ? true : false);
	tFS -> setConsiderZAxis (dScnM.posZ () > 0 ? true : false);
}

// ---
void DRAGONWIND::PlayingControlGameState::onExit ()
{
	QGAMES::GameStateControl::onExit ();

	DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (dG); // Just in case...

	dG -> setSceneVisited (dG -> activeWorld () -> activeScene () -> id ()); // The scene has already been visited...

	dG -> activeWorld () -> finalize ();
	dG -> setWorld (__MININT__); // No world any more...

	dG -> mainScreen () -> setFix (NULL); // To follow none anymore...
	dG -> mainScreen () -> setPosition (QGAMES::Position::_cero); // At the beginning...

	dG -> removeScoreObjects ();

	_mainArtist = NULL;
}

// ---
int DRAGONWIND::PlayingControlGameState::circunstanceWhen (QGAMES::GameState* st, const QGAMES::Event& evnt)
{
	int result = 0;

	if (evnt.code () == __QGAMES_GAMENESTEDSTATEFINISHED__)
	{
		switch (st -> type ())
		{
			case __DRAGONWIND_BEFOREPLAYINGGAMESTATE__:
				{
					DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
					assert (dG); // Just in case...it should be
					DRAGONWIND::Scene* cS = 
						dynamic_cast <DRAGONWIND::Scene*> (dG -> activeWorld () -> activeScene ());
					assert (cS); // Just in case...it should be but depends on the implementation.

					// Visted already? ...give control. Not and not fundamental?...give control too
					result = dG -> sceneVisited (cS -> id ()) ? 1 : (cS -> fundamental () ? 0 : 1); 
				}
				
				break;

			case __DRAGONWIND_PLAYINGGAMESTATE__:
				{
					DRAGONWIND::PlayingGameState* gS = dynamic_cast <DRAGONWIND::PlayingGameState*> (st);
					assert (gS);

					switch (gS -> lastMainCharacterState ())
					{
						case DRAGONWIND::PlayingGameState::MainCharacterState::_REACHGOAL:
							{	
								DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
								assert (dG); // Just in case...it should be
								DRAGONWIND::Scene* cS = 
									dynamic_cast <DRAGONWIND::Scene*> (dG -> activeWorld () -> activeScene ());
								assert (cS); // Just in case...

								// Not completed before?...then celebration, 
								// ...otherwise moves to the next asap!
								result = 3;
								if (gS -> lastMainCharacterGoalReached () == __DRAGONWIND_MAINGOAL__ &&
									(cS -> fundamental () && !dG -> sceneCompleted (cS -> id ())))
								{
									dG -> setSceneCompleted (cS -> id (), true); // Just completed...

									result = 1;
								}
							}

							break;

						case DRAGONWIND::PlayingGameState::MainCharacterState::_OPTIONS:
							result = 2; // Asking for options to continue...
							break;

						default:
							break; // In the rest of the cases, the default option is enough...
					}
				}

				break;

			case __DRAGONWIND_OPTIONSWHENPLAYINGGAMESTATE__:
				{
					DRAGONWIND::OptionsWhenPlayingGameState* gS =
						dynamic_cast <DRAGONWIND::OptionsWhenPlayingGameState*> (st);
					assert (gS);

					switch (gS -> lastOptionSelected ())
					{
						case __DRAGONWIND_GUISYSTEMEXITGAMEWDT__:
							result = 1; // To exit...
							break;

						default:
							break; // In the rest of the cases, the default option is enough...
					}
				}

				break;

			default:
				break; // Standard result is enough...
		}
	}

	return (result);
}

// ---
void DRAGONWIND::PlayingGameState::onEnter ()
{
	QGAMES::GameState::onEnter ();

	DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (dG); // Just in case...
	_mainArtist = dG -> mainArtist ();
	assert (_mainArtist); // Just in case...

	observe (_mainArtist);
	_mainArtist -> setLosingEnergy (true); // He/She has to...unless other thing is said!!

	_lastMainCharacterState = DRAGONWIND::PlayingGameState::MainCharacterState::_MOVING;
	_lastMainCharacterGoalReached = -1; // Meaning none...

	QGAMES::InputHandler* iH = game () -> inputHandler ();
	if (!iH)
		return;
	observe (iH);

	// Now the collisions can be detected...
	dG -> setDetectCollisions (true);
	
	// The _COUNTERSECONDS loop (created at construction time of any AdvacedArcadeGame)
	// will also be used to update the number of seconds the main entity stands in the scene (@see processEvent method in this class)
	// That number of seconds is kept in the configuration object of the game, 
	// and it is set to 0 when that object is instatiated first time or cleaned (@see @adjustPlayers in DRAGONWIND::Game)
	observe (dG -> loopCounter (DRAGONWIND::Game::_COUNTERSECONDS));
}

// ---
void DRAGONWIND::PlayingGameState::updatePositions ()
{
	QGAMES::GameState::updatePositions ();

	// If something happens before the game really begins and the main artist is dead at that point...
	// e.g a lightning killed the main artist during the count down!
	if (_mainArtist -> isDied ())
	{
		_lastMainCharacterState = DRAGONWIND::PlayingGameState::MainCharacterState::_DIED;
		notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this)); // To finish the state...
	}
}

// ---
void DRAGONWIND::PlayingGameState::onExit ()
{
	QGAMES::GameState::onExit ();

	DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
	assert (dG); // Just in case...
	dG -> setDetectCollisions (false);

	unObserve (dG -> loopCounter (DRAGONWIND::Game::_COUNTERSECONDS));

	QGAMES::InputHandler* iH = game () -> inputHandler ();
	if (!iH)
		return;
	unObserve (iH);

	_mainArtist -> setLosingEnergy (false); // Just in case...
	unObserve (_mainArtist);

	_mainArtist = NULL;
}

// ---
void DRAGONWIND::PlayingGameState::processEvent (const QGAMES::Event& evnt)
{
	switch (evnt.code ())
	{
		// When the main character says that he is dead!!
		case __DRAGONWIND_CHARACTERHASDIED__:
			{
				if (evnt.data () == (void*) _mainArtist)
				{
					_lastMainCharacterState = DRAGONWIND::PlayingGameState::MainCharacterState::_DIED;
					notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this)); // To finish the state...
				}
			}

			break;

		// When the main character has reached his goal!!
		case __DRAGONWIND_NINJAREACHEDGOAL__:
			{
				if (evnt.data () == (void*) _mainArtist)
				{
					_lastMainCharacterState = DRAGONWIND::PlayingGameState::MainCharacterState::_REACHGOAL;
					_lastMainCharacterGoalReached = evnt.values ().
						openValue (__DRAGONWIND_NUMBERGOALPARAM__).intValue (); // take care...
					notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this)); 
					// To finish the state...
				}
			}

			break;

		// When a key has been pressed in the input handler...
		// For something to happen, that key has to be either ESC (then the options pannel appears but nothings stops)
		// or the pause / continue key (then the game stops or continue)
		case __QGAMES_KEYBOARDEVENT__:
			{
				QGAMES::AdvancedArcadeGame::Conf* gCfg = 
					dynamic_cast <QGAMES::AdvancedArcadeGame::Conf*> (game () -> configuration ());
				assert (gCfg); // Just in case...(to look for the right key later)

				QGAMES::KeyBoardEventData* dt = (QGAMES::KeyBoardEventData*) evnt.data ();
				if (dt -> _internalCode == QGAMES::KeyCode::QGAMES_ESCAPE && !dt -> _on) // ESC for options
				{
					_lastMainCharacterState = DRAGONWIND::PlayingGameState::MainCharacterState::_OPTIONS;
					notify (QGAMES::Event (__QGAMES_GAMENESTEDSTATEFINISHED__, this)); // This state has to finish...
				}
				else
				if (dt -> _internalCode == gCfg -> controlKeys ()[__QGAMES_KEYPAUSE__] && !dt -> _on) // PAUSE to pause
				{
					// Stops or continue the game...
					if (game () -> isGamePaused ()) game () -> continueGame ();
					else game () -> pauseGame ();
					// ...and the main artist stops or continue losing energy in consequence...
					_mainArtist -> setLosingEnergy (!_mainArtist -> losingEnergy ());
				}
				else
				if (dt -> _internalCode == gCfg -> controlKeys ()[__QGAMES_KEYSAVE__] && !dt -> _on) // Save to pause
				{
					DRAGONWIND::Game::Conf* conf = 
						dynamic_cast <DRAGONWIND::Game::Conf*> (game () -> configuration ());
					assert (conf); // It must be a DRAGONWINGAME
					conf -> save (game () -> parameter (__GAME_DATADIRPROPERTYNAME__) + 
						std::string (__PATH_SEPARATOR__) + QGAMES::StandardInitialSelectionOptionsGameState::_CONFFILENAME);
				}
			}

			break;

		// ---
		// Now a set of events comming from the specialized input event handler...
		// Now the group of events related with how to control the main artist...
		case __DRAGONWIND_MOVINGUPEVENT__:
			{
				_mainArtist -> toClimb (QGAMES::Vector (__BD 0, __BD -1, __BD 0)); // Pure climbing movement if not before...
				_mainArtist -> toMove  (QGAMES::Vector (__BD 0, __BD -1, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGRIGHTEVENT__:
			{
				_mainArtist -> toMove  (QGAMES::Vector (__BD 1, __BD 0, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGUPRIGHTEVENT__:
			{
				// A movement like this can be no really "wished"
				// e.g, when the player is using a joystick is esy to move the the joystick up right when only
				// moving to the right is really the movement wanted.
				// This is the reason to divide the action in two: To climb that only will happen when it is possible
				// ..and to move to the right that is possible in most of the curcunstances (even flying).
				_mainArtist -> toClimb (QGAMES::Vector (__BD 1, __BD -1, __BD 0));
				_mainArtist -> toMove  (QGAMES::Vector (__BD 1, __BD -1, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGDOWNEVENT__:
			{
				_mainArtist -> toClimb (QGAMES::Vector (__BD 0, __BD 1, __BD 0)); 
				_mainArtist -> toMove  (QGAMES::Vector (__BD 0, __BD 1, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGDOWNRIGHTEVENT__:
			{
				_mainArtist -> toClimb (QGAMES::Vector (__BD 1, __BD 1, __BD 0));
				_mainArtist -> toMove  (QGAMES::Vector (__BD 1, __BD 1, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGLEFTEVENT__:
			{
				_mainArtist -> toMove  (QGAMES::Vector (__BD -1, __BD 0, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGUPLEFTEVENT__:
			{
				_mainArtist -> toClimb (QGAMES::Vector (__BD -1, __BD -1, __BD 0));
				_mainArtist -> toMove  (QGAMES::Vector (__BD -1, __BD -1, __BD 0));
			}

			break;

		case __DRAGONWIND_MOVINGDOWNLEFTEVENT__:
			{
				_mainArtist -> toClimb (QGAMES::Vector (__BD -1, __BD 1, __BD 0));
				_mainArtist -> toMove  (QGAMES::Vector (__BD -1, __BD 1, __BD 0));
			}

			break;

		case __DRAGONWIND_NOMOVINGEVENT__:
			{
				_mainArtist -> toMove  (QGAMES::Vector::_cero); // Stop?
			}

			break;

		case __DRAGONWIND_FIREEVENT__:
			{
				DRAGONWIND::InputHandlerBehaviour::NotificationData* dt = 
					(DRAGONWIND::InputHandlerBehaviour::NotificationData*) evnt.data ();
				_mainArtist -> toShoot (dt ->  _impulse, dt ->  _upOn); 
				if (dynamic_cast <DRAGONWIND::Ninja*> (_mainArtist))
					((DRAGONWIND::Ninja*) _mainArtist) -> toSwim // Just swmin when there are impulse...
						(dt -> _impulse, QGAMES::Vector (__BD (dt -> _leftOn ? -1 : (dt -> _rightOn ? 1 : 0)),
														 __BD (dt -> _upOn ? -1 : -0.25), __BD 0));
			}

			break;

		case __DRAGONWIND_JUMPEVENT__:
			{
				_mainArtist -> toJump (
					((DRAGONWIND::InputHandlerBehaviour::NotificationData*) evnt.data ()) -> _impulse,
					((DRAGONWIND::InputHandlerBehaviour::NotificationData*) evnt.data ()) -> _downOn); 
				// Could mean different things like jumping itself ot even slidding
			}

			break;

		case __DRAGONWIND_CATCHLEAVESOMETHINGEVENT__:
			{
				if (dynamic_cast <DRAGONWIND::Ninja*> (_mainArtist))
					((DRAGONWIND::Ninja*) _mainArtist) -> toCatchOrLeaveSomething ();
			}

			break;

		case __DRAGONWIND_ITERATEELEMENTSEVENT__:
			{
				if (dynamic_cast <DRAGONWIND::Ninja*> (_mainArtist))
					((DRAGONWIND::Ninja*) _mainArtist) -> toIteratePocket ();
			}

			break;
		// ---

		// Observing the loop just to add seconds to the time that the player has been playing
		// When this happen, then a notification happens there an the score could be actualized!
		case __QGAMES_LOOPCOUNTEREVENT__:
			{
				DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
				assert (dG); // Just in case...
				dG -> addToPlayerSeconds (1); // To the current player...
			}

			break;

		default:
			QGAMES::GameState::processEvent (evnt);
			break;
	}
}

// ---
void DRAGONWIND::OptionsWhenPlayingGameState::onEnter ()
{
	QGAMES::SelectionOptionsGameState::onEnter ();

	observe (game () -> inputHandler ());
}

// ---
void DRAGONWIND::OptionsWhenPlayingGameState::onExit ()
{
	unObserve (game () -> inputHandler ());

	QGAMES::SelectionOptionsGameState::onExit ();
}

// ---
void DRAGONWIND::OptionsWhenPlayingGameState::processEvent (const QGAMES::Event& evnt)
{
	QGAMES::SelectionOptionsGameState::processEvent (evnt);

	// If the ESC key is press, the the state finishes...
	if (evnt.code () == __QGAMES_KEYBOARDEVENT__)
	{
		QGAMES::KeyBoardEventData* dt = (QGAMES::KeyBoardEventData*) evnt.data ();
		if (dt -> _internalCode == QGAMES::KeyCode::QGAMES_ESCAPE && !dt -> _on)
			onOffSwitch (_SWITCHFINISH) -> set (true);
	}
}

// ---
QGAMES::SelectionOptionsGameState::Menus DRAGONWIND::OptionsWhenPlayingGameState::createMenus ()
{
	QGAMES::SelectionOptionsGameState::Menus result;
	result.insert (QGAMES::SelectionOptionsGameState::Menus::value_type 
		(__DRAGONWIND_OPTIONWHENPLAYINGGUISYSTEM__, QGAMES::SelectionOptionsGameState::Menu 
			(__DRAGONWIND_OPTIONWHENPLAYINGGUISYSTEM__, QGAMES::Position (__BD 10, __BD 10, __BD 0))));
	return (result);
}

// ---
int DRAGONWIND::OptionsWhenPlayingGameState::SelectionHandler::whenWidgetSelected (QGAMES::Widget* wdt)
{
	DRAGONWIND::Game::Conf* conf = 
		dynamic_cast <DRAGONWIND::Game::Conf*> (owner () -> game () -> configuration ());
	assert (conf); // It must be a DRAGONWINGAME

	int result = 0; // To continue...

	switch (wdt -> id ())
	{
		case __DRAGONWIND_GUISYSTEMEXITGAMEWDT__:
		case __DRAGONWIND_GUISYSTEMCONTINUEGAMEWDT__:
			result = wdt -> id (); // Time to exit the menu...with the option selected.
			break;

		case __DRAGONWIND_GUISYSTEMSAVEGAMEWDT__:
			conf -> save (owner () -> game () -> parameter (__GAME_DATADIRPROPERTYNAME__) + 
				std::string (__PATH_SEPARATOR__) + QGAMES::StandardInitialSelectionOptionsGameState::_CONFFILENAME);
			// ...but it continues in the menu...
			owner () -> game () -> sound (__QGAMES_SAVEWAVSOUND__) -> play (__QGAMES_GAMESTATESOUNDCHANNEL__);
			break;

		default:
			assert (0); // It shouldn't be here...It means there is options not controlled!!
			break;
	}

	return (result);
}

// ---
int DRAGONWIND::GameStateControl::circunstanceWhen (QGAMES::GameState* st, const QGAMES::Event& evnt)
{
	int result = 0;

	if (evnt.code () == __QGAMES_GAMENESTEDSTATEFINISHED__)
	{
		DRAGONWIND::Game* dG = dynamic_cast <DRAGONWIND::Game*> (game ());
		assert (dG); // Just in case...it should be

		switch (st -> type ())
		{
			// When the state has finishes because an options has been selected...
			case __DRAGONWIND_MENUGAMESTATE__:
				{
					QGAMES::StandardInitialSelectionOptionsGameState* gSS =
						dynamic_cast <QGAMES::StandardInitialSelectionOptionsGameState*> (st);
					assert (gSS); // Just in case there is mistake at some point of the code not totally controlled...
					switch (gSS -> lastOptionSelected ())
					{
						case __QGAMES_STDGUISYSTEMSTARTGAMEWDT__: 
							result = 0; // Start the game...
							break;

						case __QGAMES_STDGUISYSTEMEXITWDT__: 
							result = 1; // Exit the game...
							break;

						case __QGAMES_STDGUISYSTEMHALLOFFAMEWDT__: 
							result = 2; // Hall of fame...
							break;

						case __DRAGONWIND_GUISYSTEMINSTRUCTIONSWDT__: 
							result = 3; // Instructions
							break;

						default:
							assert (0); // It shouldn't be here...It means there is an option not well managed!!
							break;
					}
				}

				break;

			// When the playing control finishes...
			case __DRAGONWIND_CONTROLPLAYINGGAMESTATE__:
				{
					// The playing has finished, but depending on its final state
					// What to do will be different...
					DRAGONWIND::PlayingControlGameState* cPS = 
						dynamic_cast <DRAGONWIND::PlayingControlGameState*> (st);
					assert (cPS); // Just in case...it shouldn't happen but who knows!
					DRAGONWIND::PlayingGameState* pS = cPS -> playingState ();
					assert (pS); // Same!...

					switch (pS -> lastMainCharacterState ())
					{
						case DRAGONWIND::PlayingGameState::MainCharacterState::_DIED:
							result = 0; // The main character has died...
							break;

						case DRAGONWIND::PlayingGameState::MainCharacterState::_REACHGOAL:
							{
								// At this point the world and scene set in the game is null
								// When the playing control game state finishes, the current world is set to null...
								int nW = -1; int nS = -1; int cP = 0;
								dG -> nextWorldAndScene (dG -> currentWorld (), dG -> currentScene (), 
									pS -> lastMainCharacterGoalReached (), nW, nS, cP);

								__WLOGPLUS (std::string ("Next World, Scene & Point:") + 
									std::to_string (nW) + std::string (",") + 
									std::to_string (nS) + std::string (",") +	
									std::to_string (cP));

								if (nW == -1 && nS == -1) 
									result = 3; // No more scene, the player wins!!
												// The hall of fame has to be actualized there (when the win message is shown)
												// At this point neither the playing state has been finalized yet not the ninja
												// so the score has been actualized and the actualizedHallOfFame would be uncomplete!
								else
								{
									// The scenes are set thinking for the next state...
									dG -> setCurrentWorld (nW);
									dG -> setCurrentScene (nS);
									dG -> setCurrentInitPoint (nS, cP);

									result = 1; // Move to the next...
								}
							}

							break;

						case DRAGONWIND::PlayingGameState::MainCharacterState::_OPTIONS:
							result = 2; // The main character is asking for options to continue...
							break;

						default:
							assert (0); // It shouldn't be here. It means there is an state not weel managed!!
							break;
					}
				}

				break;

			// When the state showing that player has died finishes, it needed to take a decision...
			case __DRAGONWIND_PLAYERDIEDGAMESTATE__:
				{
					dG -> setPlayerEnergy (100); 
					// Full for the next time...always
					 
					int cPlayer = dG -> currentPlayer ();
					dG -> setLives (dG -> lives () - 1);
					do dG -> setNextPlayer (); 
					while (dG -> lives () == 0 && cPlayer != dG ->  currentPlayer ());
					result = (dG -> lives () == 0 && cPlayer == dG -> currentPlayer ()) ? 1 : 0;
					// 1 finishes the game, 0 starts back again with a new player...
				}

				break;

			// When the state of "game over" finished...
			// It is time to actualize the hall of fame!
			// The playing state will have finished and also the ninja and then the score will have been actualized!
			case __DRAGONWIND_PLAYERGAMEOVERGAMESTATE__:
				{
					dG -> actualizeHallOfFame ();

					result = 0;
				}

				break;

			default:
				// Standard result is enough...
				break;
		}
	}

	return (result);
}
