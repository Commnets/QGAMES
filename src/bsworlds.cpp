...
// ---
BATTLESHIP::World::World (int c, const QGAMES::Scenes& scns, const QGAMES::WorldProperties& prps)
			: QGAMES::World (c, scns, prps),
			  _fullConfPerPlayer ()
{
	for (QGAMES::Scenes::const_iterator i = _scenes.begin (); i != _scenes.end (); i++)
		assert (dynamic_cast <BATTLESHIP::Scene*> ((*i).second));
	// To be sure all are DRAGONWIND scenes...
}

// ---		
QGAMES::SetOfOpenValues BATTLESHIP::World::runtimeValues () const
{ 
	const BATTLESHIP::Game* dG = dynamic_cast <const BATTLESHIP::Game*> (game ());
...
// ---
std::string BATTLESHIP::World::name () const
{
	return (existsProperty (__BATTLESHIP_WORLDATTRNAME__) 
		? property (__BATTLESHIP_WORLDATTRNAME__) : __BATTLESHIP_WORLDDEFATTRNAME__);
}

// ---
void BATTLESHIP::World::initialize ()
{
	QGAMES::World::initialize ();

	BATTLESHIP::Game* dG = dynamic_cast <BATTLESHIP::Game*> (game ());
	assert (dG);
	BATTLESHIP::Game::Conf* cfg = dynamic_cast <BATTLESHIP::Game::Conf*> (dG -> configuration ());
...
// ---
void BATTLESHIP::World::finalize ()
{
